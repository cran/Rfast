  ...
  if(parallel){
      ...
      #pragma omp parallel if(parallel)
      {
        ...
        for(int j = 0; j<d;j++){
          ...
          ret(j,2) = n * log(b2) - n * b2 * log(ret(j,1) ) + (b2 - 1) * n * mlx - sum(my_pow2(conv_to<vec>::from(x/ret(j,1) ),y,b2,n));
        }
      #ifdef _OPENMP
      }
      #endif
...
List weibull_mle(NumericVector X, const double tol, const int maxiters){
  ...
  l["loglik"] = n * log(b2) - n * b2 * log(theta) + (b2 - 1) * n * mlx - sum(my_pow2(conv_to<vec>::from(x/theta),y,b2,n));
  vec param(2);
  param[0] = b2;
  param[1] = theta;
