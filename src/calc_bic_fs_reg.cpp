...
static Rcpp::NumericMatrix finalize(Rcpp::IntegerVector& idxs, 
...
static bool update_vals_end(Rcpp::NumericVector& min_min_col, std::vector<bool>& used_cols,  
...
// Alters ms
static bool calc_base(Rcpp::NumericVector& y, Rcpp::NumericMatrix& ds, const bool is_logistic, 
...
Rcpp::NumericMatrix calc_bic_fs_reg(Rcpp::NumericVector& y, Rcpp::NumericMatrix& ds, 
	...
	end = calc_base(y, ds, is_logistic, used_cols, prev_dist, tol, log_n, step, ms, idxs, bics);
	++step;
	if (end) {
		db_print("end\n");
		return finalize(idxs, bics, y, ms.add);
	}
...
static bool calc_base(Rcpp::NumericVector& y, Rcpp::NumericMatrix& ds, const bool is_logistic, 
	...
	db_print("Updating values.\nChecking end conditions.\n");
	return update_vals_end(min_min_col, used_cols, prev_dist, tol, log_n, step, 
			idxs, bics);
}
...
static Rcpp::NumericMatrix form_cmat(Rcpp::NumericMatrix& ds,
	...
	for (int i = 0; i < rows.size(); ++i) {
		for (int j = 0; j < cols.size(); ++j) {
			formed_ds.at(i, j) = ds.at(rows[i], cols[j]);
		}
	}
	return formed_ds;
}
...
static Rcpp::NumericMatrix finalize(Rcpp::IntegerVector& idxs, Rcpp::NumericVector& bics, 
