ttests <- function(x, y = NULL, ina, paired = FALSE, logged = FALSE, parallel = FALSE) {
  ...
  if ( !paired ) {
    ...
    m1 <- colmeans(x1, parallel = parallel)
    m2 <- colmeans(x2, parallel = parallel)
    f1 <- colVars(x1, suma = n1 * m1, parallel = parallel) / n1
    f2 <- colVars(x2, suma = n2 * m2, parallel = parallel) / n2
    fac <- f1 + f2
    ...
    m <- colmeans(z, parallel = parallel)
    s <- colVars(z, suma = n * m, std = TRUE, parallel = parallel)
    stat <- sqrt(n) * m / s
    if ( logged ) {
