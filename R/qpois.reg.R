qpois.reg <- function (x, y, full = FALSE, tol = 1e-09) {
    x <- model.matrix(y ~ ., data.frame(x))
    mod <- .Call("Rfast_qpois_reg", x, y, sum(y * log(y), na.rm = TRUE), 
        tol)
    res <- list(be = mod$be, devi = mod$deviance, varb = mod$phi * chol2inv(chol(mod$L2)), phi = mod$phi)
    if (full) {
        be <- mod$be
        varb <- mod$phi * chol2inv(chol(mod$L2))
